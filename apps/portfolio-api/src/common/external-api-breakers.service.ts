...
export type ExternalApiProvider = 'fmp' | 'massive' | 'twelvedata';

const BREAKER_OPTIONS = {
  // No `timeout` here deliberately — each call site already sets its own axios
  // timeout (they vary: 3s-10s depending on urgency), and opossum racing a
  // second timeout against that would just duplicate/shadow it. The breaker
 ...
 * no separate instrumentation layer needed.
 */
@Injectable()
